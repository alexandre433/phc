...
| [`design-decisions.md`](./design-decisions.md) | Rationale for every locked and provisional decision (D-001…D-051 as of 2026-05-29) |
...
- **Phase 5 (#6)** codegen + runtime — C-emit path live; Result/Option/`?` (C6), inline lambdas (C5a), stored lambdas via D-024, shared + `&flip` borrow codegen with caller write-back (D-005), `??` null-coalesce for nullable references (D-051). Every main-bearing `examples/*.phc` is native-build CI-gated.
